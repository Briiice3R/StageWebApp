...
import nafSections from '@/data/nafSections.json';
import { NafSection } from '@/types';
// ============================================
// DONNÉES NAF
// ============================================
...
type NafActivityFilterProps = {
    onSectionsChange: (sections: NafSection[]) => void;
    error?: string;
};
...
export default function NafActivityFilter({ onSectionsChange, error }: NafActivityFilterProps) {
    const [selectedSections, setSelectedSections] = useState<NafSection[]>([]);

    const handleToggle = (section: NafSection) => {
    ...
    const handleSelectAll = () => {
        const allCodes:NafSection[] = nafSections.map(s => s);
    ...
    const getDisplayText = () => {
        ...
        if (selectedSections.length === nafSections.length) {
            return 'Toutes les catégories';
        }
        if (selectedSections.length === 1) {
            return `${selectedSections[0].id} - ${selectedSections[0].libelle}`;
        }
        return `${selectedSections.length} secteurs sélectionnés`;
    };
    ...
    return (
        <Field className="w-full">
            ...
            <p className="mt-1 text-xs text-gray-500">
                Filtrez par secteurs d'activité (optionnel)
            </p>
        </Field>
    );
}
